...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div className="mt-2">
      <div className="flex gap-1 mb-1">{[0,1,2,3].map(i => <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i < strength ? colors[strength - 1] : "bg-gray-200"}`} />)}</div>
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signup({ name: form.name, email: form.email, password: form.password, confirmPassword: form.confirmPassword, phone: form.phone || undefined });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="w-full max-w-md">
        ...
        <p className="text-gray-500 mb-8">Join Capital Lab and start your CA journey today.</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Full Name *</label>
            <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Rahul Sharma"
          ...
          <div>
            ...
            <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="rahul@example.com"
              className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-navy/30 focus:border-brand-navy transition-colors bg-white" />
          </div>
          <div>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" checked={terms} onChange={e => setTerms(e.target.checked)} className="mt-0.5 accent-brand-navy w-4 h-4 shrink-0" />
            <span className="text-sm text-gray-500">
              I agree to the{" "}
              <Link href="/terms" className="text-brand-navy hover:text-brand-gold transition-colors font-medium">Terms of Use</Link>{" "}
              and{" "}
              <Link href="/privacy" className="text-brand-navy hover:text-brand-gold transition-colors font-medium">Privacy Policy</Link>.
            </span>
          </label>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ...
          <Link href="/login" className="text-brand-navy font-semibold hover:text-brand-gold transition-colors">Sign in →</Link>
        </p>
      </div>
    </div>
  );
